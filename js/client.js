    ...
    var nameU, email, photoUrl, uid, emailVerified;
      ...
      /* const getClient = () => db.collection(usuario).doc("listClientes").get();
     
     // .collection('listClientes').get(); //lo que se le dice es que desde la base de datos de firebase, obtenga todas las "tasks"   
      //importante, hay que ponerle el nombres a como aparece en la colección de firebase 
 
 
     const onGetClient = (callback) => db.collection(usuario).doc("listClientes").onSnapshot(callback);
     //.collection('listClientes').onSnapshot(callback);
 
     const deleteClient = (id) => db.collection(usuario).doc("listClientes").doc(id).delete();
     //.collection('listClientes').doc(id).delete();
 
     const getClients= (id) => db.collection(usuario).doc("listClientes").doc(id).get();
    // .collection('listClientes').doc(id).get();
 
     const updateClient= (id, updatedClient) => db.collection(usuario).doc("listClientes").doc(id).update(updatedClient).then(() => {
       Swal.fire({
         title: 'Registro Completo',
         text: "Desea visualizar la lista de clientes?",
         icon:'success',
    ...
    const saveClient = (cedula, nombre, telefono, direccion, correo, fecha) => //se crea en forma de función para facilitar las cosas
       ...
       /*  const saveClient = (cedula, nombre, telefono, direccion, correo, fecha) => //se crea en forma de función para facilitar las cosas
        ...
        db.collection(usuario).doc("listClientes").set({ //el async-await, es para decir que va a tomar tiempo para que este codigo responda
            cedula,
            nombre,
            telefono,
            direccion,
